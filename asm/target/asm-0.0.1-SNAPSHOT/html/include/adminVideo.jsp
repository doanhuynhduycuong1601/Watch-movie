	...
	<div style="width: 100%;" class="tab-content" id="v-pills-tabContent">
		<div class="tab-pane fade show active row" id="listVideo"
			...
			<c:forEach var="list" items="${listVideo}">
				<div class="row mb-4">
					<a class="col-4" href="/asm/detailVideo/${list.idVideo }">
						...
						<div class="text-white">
							...
							<c:forEach var="listGenreV" items="${list.genres}">
								<a href="/asm/search/genre/${listGenreV.id}/1"
									...
									class="text-white">${listGenreV.names}</a>
							</c:forEach>
						</div>
		...
		<div class="tab-pane fade" id="v-pills-messages" role="tabpanel"
			...
			<form action="${url}update" class="row mb-4" method="post"
				...
				<input style="display: none" name="idVideo" value="${editVideo.idVideo}">
				<div class="col-4">
					<label class="row" for="managerVideoInput"> <img src="/asm/img/imgposter/${editVideo.poster}"
						class="row" style="height: 300px;" id="managerVideoImg">
				...
				<div class="col-7">
					<div class="row mb-4">
						<label class="col-3">Name</label> <input value="${editVideo.title}" name="title"
							class="col-7" placeholder="Title?">
					</div>

					<div class="row mb-4">
						<label class="col-3">Time</label> <input name="times"
							class="col-7" placeholder="Time?" value="${editVideo.times}">
					</div>

					<div class="row mb-4">
						<label class="col-3">Exists</label>
						<div class="col-7">
							<label> <input ${editVideo.active ? 'checked="checked"' : ''} type="radio"
								name="active" value="1">Active
							</label> <label> <input ${editVideo.active ?  '' : 'checked="checked"'} type="radio" name="active" value="0">Deleted
							</label>
						</div>
					</div>
					...
					<div class="row mb-4">
						<label class="col-3">Country</label> <select class="col-7"
							...
							<c:forEach var="list" items="${country}">
								<option ${editVideo.country.idCountry ==  list.idCountry ? 'selected' : ''} value="${list.idCountry}">${list.nameCountry}</option>
							</c:forEach>
						</select>
					</div>
					...
					<div class="row mb-4">
						...
						<div class="col-5" id="genreShow"></div>
						<select class="col-3" id="genreSelect">
							<c:forEach var="genre" items="${genre }">
								<option value="${genre.id}">${genre.names}</option>
							</c:forEach>
						</select>
						<label class="col-1 btn btn-primary" onclick="addGenre()">Add</label>
					</div>
						...
						<div class="col-8" id="linkShow">
							<c:forEach var="link" items="${editVideo.links}">
								<label style="margin-right: 10px;cursor: pointer;">
									<input onclick="linkRemove(this)" name="link" value="${link.urlVideo }" 
									type="checkbox" hidden checked="checked">${link.urlVideo}
								</label>
						</c:forEach>
						</div>
