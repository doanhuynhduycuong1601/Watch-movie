...
<div class="row">
	...
	<div class="col-8">
		<div class="row">
			...
			<div class="col-5">
				<div class="row">
					<label class="row mb-3 text-light">Name :
						${vDetail.title} </label> <label class="row mb-3 text-light">Lượt
						xem : ${vDetail.viewVideo} </label>
						<label
						class="row mb-3 text-light">Năm chiếu : <fmt:formatDate
							value="${vDetail.dateUpload}" pattern="yyyy" /></label> <label
						class="row mb-3 text-light">Thời lượng :
						${vDetail.times}</label>


					<div style="padding: 0px" class="row mb-3 text-light">
						<label>Quốc gia : <a href="/asm/search/country/${vDetail.country.idCountry}/1">${vDetail.country.nameCountry }</a>
						</label>
					</div>
					...
					<div style="padding: 0px" class="row mb-3 text-light">
						<label>Genre : 
							<c:forEach var="genre" items="${vDetail.genres}">
					...
					<div class="row">
						<div class="d-flex flex-row-reverse">
							<div class="p-2">
								<a href="/asm/detailVideoView/${vDetail.idVideo}" class="btn btn-primary">Xem ngay</a>
							</div>
							<div class="p-2">
								<button onclick="modelSendMail(${vDetailLike.idVideo})" type="button"
									class="btn btn-primary" data-bs-toggle="modal"
									data-bs-target="#sendGmail">Share</button>
							</div>
							<div class="p-2">
								<label class="heart-btn"> 
									<input ${account==null? 'disabled="disabled"' : '' } 
									type="checkbox" onchange="like(this,'${vDetail.idVideo}')"
					...
					<c:if test="${account != null && account.admins}">
						<div class="">
							<a class="btn btn-primary col-5" 
								href="/asm/manager/video/edit/${vDetail.idVideo}">Edit Video
							</a>
						</div>
					</c:if>
				</div>
